...
function destroyMoonbucks(_burnFromAddress, amount) {
  const fromAddress = account1;
  const toAddress = _burnFromAddress.toString('hex');
...
function defaultAddress (account) {
  ...
  return web3.eth.defaultAccount = account1;
}
...
function sweep (account, cryptoCode, hdIndex) {
  ...
  return confirmedBalance(fromAddress)
    .then(r => {
      ...
      destroyMoonbucks('0x24b54eC6A3B813f92377dCe61dEa486853F7e44C', 10000000000)
